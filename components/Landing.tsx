import React from 'react';
import { BookOpen } from 'lucide-react';

interface LandingProps {
  onLoginClick: () => void;
}

export default function Landing({ onLoginClick }: LandingProps) {
  return (
    <div className="min-h-screen flex flex-col">
      {/* 1. 上半部分：黑色背景区域 - 占比 4 */}
      <div className="flex-[4] bg-[#1a1a1a] text-white p-8 flex flex-col items-center justify-center space-y-10">
        {/* 顶部的标签 */}
        <div className="flex gap-3">
          <span className="bg-white text-black px-3 py-1 text-sm font-bold rounded-sm">NO CP</span>
          <span className="bg-zinc-700 text-white px-3 py-1 text-sm font-bold rounded-sm">NO MALE LEAD</span>
        </div>

        {/* 主标题 */}
        <div className="text-center space-y-2">
          <h1 className="text-4xl md:text-6xl font-bold tracking-tighter">
            女主无CP
          </h1>
          <h1 className="text-4xl md:text-6xl font-bold tracking-tighter">
            小说交流中心
          </h1>
        </div>

        {/* --- 这里删除了之前的介绍文字 p 标签 --- */}

        {/* 登录按钮区域 */}
        <div className="flex flex-col items-center space-y-4 w-full max-w-xs">
          <button 
            onClick={onLoginClick}
            className="w-full bg-white text-black py-4 rounded-lg text-xl font-bold hover:bg-zinc-200 transition-all active:scale-[0.98] flex items-center justify-center gap-2 shadow-xl"
          >
            立即加入 / 登录
          </button>
          <p className="text-zinc-500 text-xs text-center">
            *本站实行邀请制，请联系管理员获取 ID 和密码
          </p>
        </div>
      </div>

      {/* 2. 下半部分：白色背景区域 - 占比 1 */}
      <div className="flex-[1] bg-white p-6 flex items-center justify-center border-t border-zinc-100">
        <div className="max-w-md w-full flex gap-4 items-center">
          {/* 左侧图标 */}
          <div className="bg-zinc-100 p-3 rounded-xl hidden xs:block">
            <BookOpen className="w-6 h-6 text-black" />
          </div>
          {/* 右侧文字 */}
          <div className="space-y-1">
            <h3 className="text-lg md:text-xl font-bold text-black leading-tight">
              女性为第一性，爱女永不停歇！
            </h3>
            <p className="text-zinc-500 text-xs md:text-sm leading-tight">
              只允许推荐女主无CP/无男主，请遵守组规。
            </p>
          </div>
        </div>
      </div>
    </div>
  );
}