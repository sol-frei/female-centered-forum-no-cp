...
export function CollectionsTab({
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {/* ── 顶部：收藏夹横向标签 + 新建按钮 ── */}
      <div className="flex items-center gap-1 overflow-x-auto pb-1 scrollbar-hide" ref={menuRef}>

        {collections.map(c => (
          <div key={c.id} className="relative flex-shrink-0">
            {renamingId === c.id ? (
              /* 重命名状态：整行变成输入框 */
              <div className="flex items-center gap-1 px-3 py-2 border-2 border-black rounded-full bg-white">
                <input
                  autoFocus
                  value={renameValue}
                ...
                <button
                  ...
                  disabled={renaming}
                  className="w-7 h-7 flex items-center justify-center rounded-full bg-green-500 text-white active:bg-green-600"
                >
                  {renaming ? <Loader2 className="w-4 h-4 animate-spin" /> : <Check className="w-4 h-4" />}
                </button>
                <button
                  onClick={() => setRenamingId(null)}
                  className="w-7 h-7 flex items-center justify-center rounded-full bg-zinc-200 text-zinc-600 active:bg-zinc-300"
                >
                  <X className="w-4 h-4" />
                </button>
              </div>
              ...
              /* 正常标签 */
              <div className={`flex items-center gap-1 pl-4 pr-2 py-2 rounded-full text-sm font-medium cursor-pointer transition-colors whitespace-nowrap select-none ${
                selectedId === c.id
                  ? 'bg-zinc-900 text-white'
                  : 'bg-zinc-100 text-zinc-600 hover:bg-zinc-200'
              }`}>
                <span onClick={() => setSelectedId(c.id)}>{c.name}</span>

                {/* ⋯ 菜单按钮 */}
                <button
                  onClick={e => { e.stopPropagation(); setMenuOpenId(menuOpenId === c.id ? null : c.id); }}
                  className={`w-6 h-6 flex items-center justify-center rounded-full transition-colors ${
                    selectedId === c.id ? 'hover:bg-zinc-700' : 'hover:bg-zinc-300'
                  }`}
                >
                ...
                {/* 下拉菜单 */}
                {menuOpenId === c.id && (
                  <div className="absolute left-0 top-full mt-2 z-50 bg-white border border-zinc-200 rounded-2xl shadow-xl py-1.5 w-36 overflow-hidden">
                    <button
        ...
        {/* 新建按钮 / 输入框 */}
        {showCreateInput ? (
          <div className="flex items-center gap-1 px-3 py-2 border-2 border-dashed border-zinc-300 rounded-full flex-shrink-0">
            <input
              autoFocus
              value={newName}
            ...
            <button
              ...
              className="w-7 h-7 flex items-center justify-center rounded-full bg-black text-white disabled:opacity-40 active:bg-zinc-700"
            >
          ...
          {posts.map((p: any) => (
            ...
            >
              ...
              <button
                ...
                className="flex-shrink-0 w-9 h-9 flex items-center justify-center rounded-xl text-zinc-300 hover:text-red-500 hover:bg-red-50 active:bg-red-100 transition-colors"
                title="取消收藏"
              >
